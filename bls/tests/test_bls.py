...
def test_input_units(data):
    ...
    with pytest.raises(units.UnitConversionError):
        BLS(t * t_unit, y * y_unit, dy * units.one)
    with pytest.raises(units.UnitConversionError):
        BLS(t * t_unit, y * units.one, dy * y_unit)
    with pytest.raises(units.UnitConversionError):
        BLS(t * t_unit, y, dy * y_unit)
    model = BLS(t*t_unit, y * units.one, dy)
    assert model.dy.unit == model.y.unit
    model = BLS(t*t_unit, y * y_unit, dy)
    assert model.dy.unit == model.y.unit
    model = BLS(t*t_unit, y*y_unit)
